...
void main()
{
    ...
    float obj_color = ambient_light + factorAtenuare * (diffuse_light + specular_light);
    // culoareObiect = ambientala + factorAtenuare * ( difuza + speculara );

    // TODO(student): Send color light output to fragment shader
    color = obj_color * object_color;
    vec3 dir = world_pos - light_position;
    vec3 new_pos = world_pos + dir / (distance(world_pos, light_position) + 0.000001);

    gl_Position = Projection * View * vec4(new_pos, 1.0);
    // vec3 dir = gl_Position - light_position;
    // gl_Position += dir * 0.5;
}
